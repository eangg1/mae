...
navigator.mediaDevices.getUserMedia({ audio: true })
    .then(function(stream) {
        ...
        setInterval(function() {
            ...
            // Increase the threshold for blowing to 300 for a stronger blow
            if (average > 300 && !isFlameOff) { 
                if (!isBlowing) {
                    isBlowing = true;
                    setTimeout(() => {
                        toggleFlame(); // Turn off the flame after a delay
                        toggleFlame2(); // Toggle flame2 text visibility
                        isFlameOff = true;
                        isBlowing = false; // Reset isBlowing after action
                    }, 500); // Adjust the delay as needed
                }
            } else {
                isBlowing = false; // Reset if not blowing
            }
        }, 100);
    })
    .catch(function(err) {
